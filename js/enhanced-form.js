 ...
 * Handles the comprehensive Digital Hermit community signup form
 */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleFormSubmit(e) {
        ...
        try {
            ...
            // Save to Firebase
            const result = await firebaseService.saveUserSignup(formData);
            
            if (result.success) {
                // Send notification
                await firebaseService.sendSignupNotification(formData);
                
                // Show success message
                showMessage('Welcome to the Digital Hermit community! Your profile has been created successfully.', 'success');
                ...
                // Track analytics
                if (window.analytics) {
                    window.analytics.logEvent('enhanced_signup_complete', {
                        interests_count: formData.interests.length,
                        has_bio: !!formData.bio,
                        has_hobbies: !!formData.hobbies,
                        work_style: formData.workStyle || 'not_specified'
                    });
                }
